        ...
        <main className="sm:flex sm:mt-16 sm:mx-20 sm:gap-10 ">
          ...
          <div className="sm:basis-2/3 w-full">
            ...
            <div className="flex gap-4 my-3">
              <button onClick={() => handleColorChange('White')} className={`rounded-full bg-slate-200  p-4 ${selectedColor === 'White' ? 'ring ring-slate-700 ring-offset-2' : ''}`}></button>
              <button onClick={() => handleColorChange('Grey')} className={`rounded-full bg-gray-700  p-4 ${selectedColor === 'Grey' ? 'ring ring-slate-700 ring-offset-2' : ''}`}></button>
              <button onClick={() => handleColorChange('Black')} className={`rounded-full bg-black p-4 ${selectedColor === 'Black' ? 'ring ring-slate-700 ring-offset-2' : ''}`}></button>
            </div>

            <h3>Size </h3>
            <div className="flex sm:gap-4 gap-3 mt-4 mb-5">
              <button onClick={() => handleSizeChange('39')} className={`rounded bg-slate-200 py-2 px-8 ${selectedSize === '39' ? 'ring ring-slate-700 ring-offset-2' : ''}`}>
                39
              </button>
              <button onClick={() => handleSizeChange('40')} className={`rounded bg-slate-300 py-2 px-8 ${selectedSize === '40' ? 'ring ring-slate-700 ring-offset-2' : ''}`}>
                40
              </button>
              <button onClick={() => handleSizeChange('41')} className={`rounded bg-slate-400 py-2 px-8 ${selectedSize === '41' ? 'ring ring-slate-700 ring-offset-2' : ''}`}>
                41
              </button>
              <button onClick={() => handleSizeChange('42')} className={`rounded bg-slate-500 py-2 px-8 ${selectedSize === '42' ? 'ring ring-slate-700 ring-offset-2' : ''}`}>
                42
              </button>
              <button onClick={() => handleSizeChange('43')} className={`rounded bg-slate-600 py-2 px-8 ${selectedSize === '43' ? 'ring ring-slate-700 ring-offset-2' : ''}`}>
                43
              </button>
            </div>

            <div className=" flex gap-5 items-center">
              <button className=" rounded py-2 px-5 bg-slate-400 hover:bg-slate-600" onClick={handleDecrement}>
                -
              </button>
              <span>{quantity}</span>
              <button className=" rounded py-2 px-5 bg-slate-400 hover:bg-slate-600" onClick={handleIncrement}>
                +
              </button>
            </div>

            <div className="flex text-white gap-4 my-4">
              <button onClick={addToCart} className=" rounded py-3 px-7 bg-black hover:bg-gray-800">
                Add To Cart
              </button>
              <Toaster />
            </div>
